...
def _module_teaching_plan(module_title, module_description):
    ...
    if "introduction to investing" in lower_title or "markets" in lower_title:
        ...
        plan.update({
            "hook": "Asset allocation is the single largest controllable driver of long-horizon return variability. Brinson, Hood, and Beebower (1986) attributed roughly 90% of pension-return variance to policy weights, leaving security selection and market timing to share the rest. In FIN235 terms: allocation is the policy layer, designed once and enforced mechanically.",
            "core_terms": [
                ("Strategic asset allocation (SAA)", "Long-term target weights set by objectives, horizon, and risk capacity; the policy benchmark for the portfolio."),
                ("Tactical asset allocation (TAA)", "A bounded, evidence-based, time-bound deviation from SAA when valuation, momentum, or macro conditions justify it."),
                ("Rebalancing band", "Pre-set drift tolerance (e.g., ±5% absolute or ±25% relative) that triggers a return to target weights."),
                ("Glidepath", "A pre-planned reduction in equity weight as the goal date approaches (e.g., target-date funds)."),
                ("Risk parity", "Allocation by equal risk contribution per sleeve rather than equal dollars."),
                ("Bucketing", "Segmenting capital by purpose and horizon (liquidity, growth, legacy) so each bucket carries appropriate risk."),
                ("Investment Policy Statement (IPS)", "Written policy specifying targets, bands, tilt limits, and review conditions; governs the next trade so the news cycle cannot."),
            ],
            "likely_confusion": "Students change SAA in response to headlines or trailing returns. SAA should change only when goals, horizon, liquidity needs, or risk capacity change—not when markets feel uncomfortable.",
            "scenario": "Your 60/30/10 (equity/bond/cash) target has drifted to 72/22/6 after a strong equity rally. Bond yields are up ~80 bps and recession headlines dominate the news. Decide your action and defend it with allocation policy, not market opinion.",
            "deep_dive_paragraphs": [
                "Strategic asset allocation (SAA) maps an investor's objectives, horizon, and risk capacity onto policy weights across asset classes. The Brinson-Hood-Beebower decomposition shows that policy weights explain the majority of cross-sectional return variability in diversified portfolios; selection and timing share the rest. The practical implication for an investments course is that the allocation decision deserves at least as much analytical effort as any individual security pick. SAA changes when the underlying inputs change—a longer horizon, a new liquidity need, a sustained shift in risk capacity, a change in goal. It does not change because of last quarter's returns or this week's macro narrative.",
                "Tactical asset allocation (TAA), or tilt, is a temporary deviation from SAA. Disciplined TAA is bounded in three dimensions: size (e.g., ±5–10% from policy weight), time (a defined window such as 6–18 months), and thesis (a written, falsifiable claim with explicit evidence). Tilts unwind when evidence weakens or the window expires—not when the position becomes uncomfortable, and not when it becomes profitable. Without those guardrails, TAA degrades into market timing dressed up as policy and quietly drifts the portfolio toward whatever is currently comfortable.",
                "Rebalancing is the mechanical restoration of policy weights. Two common rules dominate practice: calendar rebalancing trades on a fixed schedule (e.g., quarterly) regardless of drift; threshold rebalancing trades only when a sleeve breaches a pre-set band. Threshold rules empirically tend to outperform calendar rules because they trade only when the portfolio has actually drifted and they impose contrarian behavior—trimming sleeves that have appreciated and adding to sleeves that have lagged. Rebalancing is primarily a risk-control tool; any return benefit is secondary and inconsistent. Tax-aware variants minimize realized capital gains by routing new contributions, dividends, and interest into under-weighted sleeves before selling appreciated lots.",
                "Specialized SAA designs answer specific questions. A glidepath reduces equity weight as the goal date approaches—the design behind most target-date funds—so portfolio volatility falls when the horizon over which the investor can absorb a drawdown is shrinking. Risk parity allocates by equal risk contribution rather than equal dollars, typically producing a higher fixed-income weight than 60/40 because bonds are less volatile per dollar. Bucketing segments capital by purpose: a liquidity bucket in cash and short-duration Treasuries, a growth bucket in diversified equity, a legacy bucket in long-horizon assets. Each design answers the same underlying question—'What is this money for, and over what horizon?'—but in a different way.",
                "Behavioral risk is the dominant failure mode in allocation. The classic mistake is panic-selling equity during a drawdown, locking in losses, and missing the recovery the SAA was designed to endure. The mirror mistake is adding equity after an extended rally because it feels safe; that pushes effective risk above the investor's actual risk capacity right before the next drawdown. The cure is policy. A written IPS that specifies target weights, rebalancing bands, tilt limits, tax-aware sequencing rules, and the conditions under which SAA may be reviewed turns the next decision into a rule execution instead of an opinion contest.",
            ],
            "worked_example": (
                "An FIN235 simulator account holds a 60/30/10 target (equity/bond/cash) with ±5% absolute bands on a $200,000 portfolio. After a 25% equity rally with bonds flat, the portfolio is worth roughly $234,000 with weights ≈ 64.1% / 25.6% / 8.5%—every sleeve outside its band. Target dollars are $140,400 equity / $70,200 bond / $23,400 cash, versus current ≈ $150,000 / $60,000 / $20,000. Threshold rebalancing sells about $9,600 of equity and buys ≈ $10,200 of bonds and $3,400 of cash to restore target (with a small interest distribution adjusting the net). If instead the student 'lets winners run' to 72/22/6, the portfolio's expected loss in a hypothetical −25% equity drawdown rises from ≈ 15% (60% × 25%) to ≈ 18% (72% × 25%) of total value—roughly $7,000 of extra risk the investor never chose. The rebalance is a risk-control trade, not a return prediction."
            ),
            "walkthrough": [
                "Write the investor profile: goal, horizon, planned contributions/withdrawals, liquidity needs, and behavioral risk capacity (max drawdown you can hold through without selling).",
                "Set SAA target weights consistent with that profile across at least three asset classes; define rebalancing bands (e.g., ±5% absolute) and any tax-aware sequencing rules.",
                "If you intend to apply a tactical tilt, document the thesis, evidence, size limit, and unwind trigger in writing before placing the trade.",
                "On each review date, compute current weights vs target; if any sleeve is outside its band, calculate the dollar trades to restore target and execute (route new cash and dividends to under-weighted sleeves first in taxable accounts).",
                "Log every allocation decision, rule applied, and outcome in an IPS journal so future reviews can separate policy from improvisation.",
            ],
            "application_prompt": "Run the drift check on your simulator account now: list each sleeve's market value, current weight, target weight, and the trade your rebalancing policy dictates. State whether you would execute it today and why.",
            "assignment_q1": [
                "Write a one-paragraph Investment Policy Statement (IPS) for your simulator portfolio: state the goal, horizon, planned contributions or withdrawals, liquidity needs, and the maximum drawdown you believe you could hold through without selling.",
                "Propose Strategic Asset Allocation (SAA) target weights across at least three asset classes (e.g., US equity, international equity, investment-grade bonds, cash). Justify each weight using horizon and risk capacity—not trailing returns—and reference at least one correlation or risk-parity consideration in your reasoning.",
                "Define your rebalancing policy: pick calendar, threshold, or a hybrid; state the specific bands (e.g., ±5% absolute) and any tax-aware sequencing rule (e.g., route new cash and dividends to under-weighted sleeves before selling appreciated lots).",
                "Identify the single behavioral failure most likely to derail this policy under stress (e.g., panic-selling in a drawdown, performance chasing after a rally). Specify the pre-committed rule in your IPS that disarms it.",
                "State whether a tactical tilt is currently warranted. If yes, write the thesis, evidence, size limit (% of policy weight), and the trigger that would unwind it. If no, state what new evidence would change your answer.",
            ],
            "assignment_q2": [
                "Pull current sleeve weights from your simulator account for at least three asset-class sleeves. Show for each sleeve: market value, total portfolio value, and current weight = sleeve MV / total MV (formula and inputs).",
                "Compute absolute drift for each sleeve (current weight − target weight). Identify which sleeves breach a ±5% band and which would breach a ±25% relative band. Show both calculations.",
                "Compute the rebalancing trades required to restore target weights: dollar amount and direction for each sleeve. If the account is taxable, describe one tax-aware adjustment to the trade plan and the dollar impact.",
                "Estimate each sleeve's contribution to total portfolio return over a clearly defined window using Sleeve Weight × Sleeve Return. Identify the sleeve that drove the drift and explain whether the move reflects edge, broad factor exposure, or concentration.",
                "Compare your portfolio's realized risk (volatility or peak-to-trough drawdown) over the window to a 60/40 SPY/AGG benchmark you compute yourself. State one allocation change you would make on risk-capacity grounds and back it with the supporting number.",
            ],
            "quiz_bank": [
